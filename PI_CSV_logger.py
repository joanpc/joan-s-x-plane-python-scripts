...
class PythonInterface:
    def XPluginStart(self):
        ...
        # Logger floop        # floop
        self.mfloop = self.mainFloopCallback
        XPLMRegisterFlightLoopCallback(self, self.mfloop, 0, 0)
    ...
    def XPluginStop(self):
        self.stop()
        XPLMUnregisterFlightLoopCallback(self, self.mfloop, 0)
    ...
    def XPluginReceiveMessage(self, inFromWho, inMessage, inParam):
        if (inFromWho == XPLM_PLUGIN_XPLANE and inParam == XPLM_PLUGIN_XPLANE):
            ...
            if (inMessage == XPLM_MSG_PLANE_LOADED ):
                self.stop()
                self.clearConfig()
                XPLMSetFlightLoopCallbackInterval(self, self.mfloop, -1, 0, 0)
    ...
    def mainFloopCallback(self, elapsedMe, elapsedSim, counter, refcon):
        self.getConfig()
        if self.autostart:
            self.start()
